export default {
    async fetch(request, env) {
        ...
        // 只保護 /admin 開頭的路徑（後台管理室），其他公開頁面完全不受影響
        if (url.pathname.startsWith('/admin')) {
            const authHeader = request.headers.get('Authorization');

            if (!isAuthorized(authHeader, env.ADMIN_USER, env.ADMIN_PASSWORD)) {
                return new Response('本卷宗為機密檔案，請輸入帳號密碼才能調閱後台', {
                    status: 401,
                    headers: {
                        'WWW-Authenticate': 'Basic realm="Polipoli Archive Admin", charset="UTF-8"'
                    }
                });
            }
        }
        ...
        // 重新建構一個乾淨的 request（只保留網址跟方法），避免原始 request 帶著
